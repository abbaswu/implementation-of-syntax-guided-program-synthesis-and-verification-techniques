...
def bar_chart_from_dataframe(dataframe: pd.DataFrame, data_column: str, group_column: str, **kwargs):
    # 所有的组
...
# kwargs：其他传递给axes.bar()，用于绘制条形统计图的参数（如log=True）
def grouped_bar_chart_from_dataframe(dataframe: pd.DataFrame, data_column: str, group_bars_by: str, label_bars_by: str,
                                     title: str = '', label_name_replacer: Callable[[str], str] = lambda x: x,
                                     **kwargs):
    # 所有的组
...
def grouped_box_chart_from_dataframe(dataframe: pd.DataFrame, data_column: str, group_bars_by: str, label_bars_by: str, **kwargs):
    # 所有的组
    groups = dataframe[group_bars_by].unique()
    # 所有的标签
    labels = dataframe[label_bars_by].unique()
...
def grouped_average_bar_chart_from_dataframe(dataframe: pd.DataFrame, data_column: str, group_bars_by: str, label_bars_by: str, **kwargs):
    # 所有的组
    groups = dataframe[group_bars_by].unique()
    # 所有的标签
    labels = dataframe[label_bars_by].unique()
